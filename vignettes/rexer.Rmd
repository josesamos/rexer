...
To define an exam, we need an *Rmd* file with the exam template. From the template, we create an object `exam`. The communication between the object and the template is through parameters that the template must include, and whose values are set in the object and the associated definition methods.

The creation of an `exam` object with one of the templates provided in the package and the default values of the parameters is shown below.
